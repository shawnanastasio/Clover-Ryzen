...
VOID GetCPUProperties (VOID)
{
  ...
  if (gCPUStructure.Vendor == CPU_VENDOR_AMD && gCPUStructure.Extfamily >= 0x8) {
  ...
  if(gCPUStructure.Vendor == CPU_VENDOR_INTEL &&
     gCPUStructure.CPUID[CPUID_0][EAX] >= 0x15) {
  ...
  if(gCPUStructure.Vendor == CPU_VENDOR_INTEL &&
    ...
    if(gCPUStructure.Extfamily == 0x00 /* K8 */) {
      ...
      gCPUStructure.CurrentSpeed = (UINT32)DivU64x32((msr & 0xFF), (RShiftU64(msr, 8) & 0x3F))*200;
      ...
      gCPUStructure.MaxRatio = (UINT32)DivU64x32((msr & 0xFF), (RShiftU64(msr, 8) & 0x3F)) * 20;

      // Get minimum ratio
      msr = AsmReadMsr64(K17_PSTATE_DEF + MinPstate);
      gCPUStructure.MinRatio = (UINT32)DivU64x32((msr & 0xFF), (RShiftU64(msr, 8) & 0x3F)) * 20;
